> Fullstack &#9656; [Parte 1](./part1.md) &#9656; Un estado más complejo, depurando aplicaciones react
...
<details><summary>d Un estado más complejo, depurando aplicaciones React</summary>

&#9656; [Estado complejo](#Estado-complejo)

&#9656; [Manejo de matrices](#Manejo-de-matrices)
...
&#9656; [Manejo de Eventos Revisitado](#Manejo-de-Eventos-Revisitado)

&#9656; [Función que devuelve una función](#Función-que-devuelve-una-función)

&#9656; [Pasando controladores de eventos a componentes secundarios](#Pasando-controladores-de-eventos-a-componentes-secundarios)

&#9656; [No definir componentes dentro de los componentes](#No-definir-componentes-dentro-de-los-componentes)

&#9656; [Lectura útil](#Lectura-útil)

&#9656; [Ejercicios 1.6.-1.14.](#Ejercicios-1.6.-1.14.)
...
El estado del componente o una parte de su estado puede ser de cualquier tipo. Podríamos implementar la misma funcionalidad guardando el recuento de clics de los botones _left_ y _right_ en un solo objeto:
...
Ahora el componente solo tiene una parte de estado y los controladores de eventos deben encargarse de cambiar el _estado completo de la aplicación_.
...
El nuevo valor de la propiedad _left_ ahora es el mismo que el valor de _left + 1_ del estado anterior, y el valor de la propiedad _right_ es el mismo que el valor de la propiedad _right_ del estado anterior.

Podemos definir el nuevo objeto de estado un poco más claramente utilizando la sintaxis de [object spread](https://developer.mozilla.org/es/docs/Web/JavaScript/Referencia/Operadores/Sintaxis_Spread) que se agregó a la especificación del lenguaje en el verano de 2018:
...
La sintaxis puede parecer un poco extraña al principio. En la práctica, _{...clicks}_ crea un nuevo objeto que tiene copias de todas las propiedades del objeto _clicks_. Cuando especificamos una propiedad en particular, por ejemplo, _right_ en _{...clics, right: 1}_, el valor de la propiedad _right_ en el nuevo objeto será 1.
...
Hay situaciones en las que puede resultar beneficioso almacenar una parte del estado de la aplicación en una estructura de datos más compleja. [La documentación oficial de React](https://es.reactjs.org/docs/hooks-faq.html#should-i-use-one-or-many-state-variables) contiene una guía útil sobre el tema.
...
Cuando se hace clic en el botón _left_, agregamos la letra _L_ a la matriz _allClicks_:
...
La parte del estado almacenada en _allClicks_ ahora está configurada para ser una matriz que contiene todos los elementos de la matriz de estado anterior más la letra _L_. La adición del nuevo elemento a la matriz se logra con el método [concat](https://developer.mozilla.org/es/docs/Web/JavaScript/Reference/Global_Objects/Array/concat), que no muta la matriz existente, sino que devuelve una _nueva copia de la matriz_ con el elemento agregado.

Como se mencionó anteriormente, también es posible en JavaScript agregar elementos a una matriz con el método [push](https://developer.mozilla.org/es/docs/Web/JavaScript/Referencia/Objetos_globales/Array/push). Si agregamos el elemento empujándolo a la matriz _allClicks_ y luego actualizando el estado, la aplicación aún parecería funcionar:
...
Llamamos al método [join](https://developer.mozilla.org/es/docs/Web/JavaScript/Reference/Global_Objects/Array/join) en la matriz _allClicks_ que une todos los elementos en una sola cadena, separados por la cadena pasada como parámetro de función, que en nuestro caso es un espacio vacío.

## Renderizado condicional

Modifiquemos nuestra aplicación para que el renderizado del historial de clics sea manejada por un nuevo componente _History_:
...
Ahora el comportamiento del componente depende de si se ha hecho clic en cualquier botón. Si no es así, lo que significa que la matriz _allClicks_ está vacía, el componente muestra un elemento div con algunas instrucciones en su lugar:
...
El componente _History_ representa elementos React completamente diferentes según el estado de la aplicación. Esto se llama _renderizado condicional_.

React también ofrece muchas otras formas de hacer [renderizado condicional](https://es.reactjs.org/docs/conditional-rendering.html). Veremos esto más de cerca en la [parte 2](../../2/es/part2.md).
...
>
> La _Consola_, en particular, debería estar siempre abierta, a menos que haya una razón específica para ver otra pestaña.
...
**NB** Cuando use _console.log_ para depurar, no combine _objectos_ como en Java utilizando el operador más. En lugar de escribir:
...
Imprimir en la consola no es de ninguna manera la única forma de depurar nuestras aplicaciones. Puede pausar la ejecución del código de su aplicación en el _depurador_ de la consola de desarrollador de Chrome, escribiendo el comando [debugger](https://developer.mozilla.org/es/docs/Web/JavaScript/Reference/Statements/debugger) en cualquier parte de su código.

La ejecución se detendrá una vez que llegue a un punto donde se ejecuta el comando _debugger_:

![](../../images/1/7a.png)

Al ir a la pestaña _Console_, Es fácil inspeccionar el estado actual de las variables:

![](../../images/1/8a.png)

Una vez que se descubre la causa del error, puede eliminar el comando _debugger_ y actualizar la página.

El depurador también nos permite ejecutar nuestro código línea por línea con los controles que se encuentran en el lado derecho de la pestaña _Source_.

También puede acceder al depurador sin el comando _debugger_ agregando puntos de interrupción en la pestaña _Source_. La inspección de los valores de las variables del componente se puede hacer en la sección _Scope_:
...
El primer _State_ contiene el valor del estado _left_, el siguiente contiene el valor del estado _right_ y el último contiene el valor del estado _allClicks_.
...
Para que el botón reaccione a un evento de clic, tenemos que agregarle un _controlador de eventos_.
...
El problema aquí es que nuestro controlador de eventos está definido como una _llamada a función_, lo que significa que al controlador de eventos se le asigna realmente el valor devuelto por la función, que en el caso de _console.log_ es _indefinido_.
...
La variable _handleClick_ ahora está asignado a una referencia a la función. La referencia se pasa al botón como el atributo _onClick_:
...
Otra forma de definir un controlador de eventos es utilizar una _función que devuelve una función_.
...
El _valor de retorno_ de la función es otra función que se asigna a la variable _handler_.
...
El controlador de eventos se crea _ejecutando_ la llamada de función _hello('world')_. La llamada a la función devuelve la función:
...
Cuando se renderiza el componente, se crea el botón _thousand_:
...
Usar el componente _Button_ es simple, aunque debemos asegurarnos de que usamos los nombres de atributos correctos al pasar props al componente.

![](../../images/1/12e.png)

## No definir componentes dentro de los componentes

Empecemos a mostrar el valor de la aplicación en su propio componente _Display_.

Cambiaremos la aplicación definiendo un nuevo componente dentro del componente _App_.
...
En su lugar, movamos la función del componente _Display_ a su lugar correcto, que está fuera de la función del componente _App_: 
...
- Vale la pena echarle un vistazo a la [documentación oficial](https://es.reactjs.org/docs/hello-world.html) de React en algún momento, aunque la mayor parte será relevante solo más adelante en el curso. Además, todo lo relacionado con los componentes basados ​​en clases es irrelevante para nosotros;
...
_Algunos de los ejercicios funcionan en la misma aplicación. En estos casos, es suficiente enviar solo la versión final de la aplicación. Si lo desea, puede realizar un commit después de cada ejercicio terminado, pero no es obligatorio._
...
Como la mayoría de las empresas, [Unicafe](https://www.unicafe.fi/#/9/4) recopila comentarios de sus clientes. Su tarea es implementar una aplicación web para recopilar comentarios de los clientes. Solo hay tres opciones para los comentarios: _good (bueno)_, _neutral_ y _bad (malo)_.
...
Puede implementar la aplicación en un solo archivo _index.js_. Puede utilizar el siguiente código como punto de partida para su aplicación.
...
Refactorice su aplicación para que la visualización de las estadísticas se extraiga en su propio componente _Statistics_. El estado de la aplicación debe permanecer en el componente raíz _App_.
...
- _Statistic_ para mostrar una única estadística, por ejemplo, la puntuación media.

Para ser claros: el componente _Statistics_ siempre muestra una única estadística, lo que significa que la aplicación utiliza varios componentes para representar todas las estadísticas:
...
Muestra las estadísticas en una [tabla](https://developer.mozilla.org/es/docs/Learn/HTML/Tables/Basics) HTML, de modo que su la aplicación se ve más o menos así:
...
_Fuente típica de un error `Unchecked runtime.lastError: Could not establish connection. Receiving end does not exist.` es la extensión de Chrome. Intente ir a `chrome://extensions/` e intente deshabilitarlos uno por uno y actualizar la página de la aplicación React; el error debería desaparecer eventualmente._
...
Expanda la siguiente aplicación agregando un botón en el que se puede hacer clic para mostrar una anécdota _aleatoria_ del campo de la ingeniería de software:
...
[Proponer cambios en el material](https://github.com/sebastiantorres86/Fullstack-Spanish/edit/main/src/content/1/es/part1d.md)

</div>

<div align="right">
  <a href="../../2/es/part2.md">Parte 2<br>Siguiente parte</a>
</div>